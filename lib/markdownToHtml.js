  ...
  return (tree) => {
    ...
    for (const { node, index, parent } of imageNodes) {
      ...
      const imgProps = {
        ...
        loading: "lazy",
        decoding: "async",
        // No radius, no shadow - matches the site's flat, hard-edged
        // Swiss-style treatment (--radius is 0px everywhere else).
        className: "border border-(--color-border-light) w-full h-auto",
        ...(title && { title }),
      };
        ...
        parent.children[index] = {
          ...
          children: [
            imgNode,
            {
              ...
              tagName: "figcaption",
              properties: {
                // "stat-caption" is the site's real caption treatment
                // (small mono, muted, letter-spaced) - keeps this in
                // step with the design system instead of falling back
                // to Tailwind's default gray scale.
                className: "stat-caption mt-2 text-center",
              },
