    ...
    <>
      <div className="grid grid-cols-1 gap-x-6 gap-y-12 sm:grid-cols-2 lg:grid-cols-4 lg:gap-x-8 lg:gap-y-16">
        {visibleProjects.map((project, index) => (
          ...
          >
            {/* Image */}
            <div className="relative aspect-[5/3] w-full overflow-hidden border border-(--color-border-light) bg-(--color-ink-surface)">
              <Image
                src={project.image}
            ...
            <div className="mt-5 flex grow flex-col">
              ...
              {project.tech.length > 0 && (
                <span className="ui-text mt-auto pt-4 font-normal! tracking-normal! normal-case! text-(--color-muted)">
                  {project.tech.join(" / ")}
                </span>
              )}

              <span className="ui-text mt-5 flex items-center gap-2 transition-colors duration-(--duration-fast) group-hover:text-(--color-accent)">
                View Project
                <svg
                  className="h-3.5 w-3.5 transition-transform duration-(--duration-fast) group-hover:translate-x-1"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
      ...
      {hasMore && (
        <div
          ref={sentinelRef}
          className="mt-16 flex justify-center border-t border-(--color-line) pt-8"
        >
      ...
      {reachedEnd && (
        <div className="mt-16 border-t border-(--color-line) pt-8 text-center">
